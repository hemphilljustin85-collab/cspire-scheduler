...
async function sendEmail(to: string, subject: string, html: string) {
  ...
  if (!response.ok) {
    throw new Error(`Email failed for ${to}: ${await response.text()}`);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const employees = await supabaseRest<Employee[]>(
      "employees?select=id,employee_name,email,phone,notify_email,notify_sms,status&or=(status.eq.Active,status.is.null)&order=employee_name.asc",
    );
    ...
    const subject = `Work schedule published for week of ${weekStart}`;
    ...
    for (const employee of employees) {
      ...
      if (employee.notify_email !== false && employee.email) {
        try {
          await sendEmail(
            employee.email,
            subject,
            `
              <div style="font-family:Arial,sans-serif;line-height:1.5">
                <h2>Your work schedule is ready</h2>
                <p>Hello ${employee.employee_name},</p>
                <p>The schedule for the week of <strong>${weekStart}</strong> has been published.</p>
                <p>
                  <a href="${teamLink}" style="display:inline-block;padding:12px 18px;background:#2563eb;color:white;text-decoration:none;border-radius:6px">
                    View Team Schedule
                  </a>
                </p>
                <p>Direct link: ${teamLink}</p>
              </div>
            `,
          );
          emailSent += 1;
